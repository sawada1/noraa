               ...
               <div class="custom-shadow-container position-sticky top-0">
                   <div>
                       <div class="head d-flex align-items-center gap-3">
                           ...
                           <h4 class="fw-bold fc-Darkgreay fs-6"> عدد الاسألة {{ questionNum }} من 25 </h4>
                       </div>
           ...
           <div class="col-xl-9 col-lg-9 col-12">
               <div class="custom-shadow-container">
                   ...
                   <div v-if="selectedInputs == 2"  class="row">
                       <div v-for="input,index in store.quizzes?.steps?.step_2" class="col-xl-6 col-lg-6 col-12 mb-4">
                           <div class="input  d-flex flex-column gap-2">
                               <label for=""> {{ input?.name }} </label>
                               <Select  v-model="selectedStep2[`selected${index+1}`]" @update:modelValue="updateAnswer(input.id, $event)" v-if="input?.type == 'single'"  :options="input?.answers" style="min-height: 41px;" optionValue="id" optionLabel="name"
                                   placeholder="  " class="" />
                                   <InputText v-else name="" v-model="selectedStep2[`selected${index+1}`]"   @update:modelValue="updateAnswer(input.id, $event)"
                                   type="text" placeholder="" />
                   ...
                   <div v-if="selectedInputs == 3"  class="row">
                       <div v-for="input , index in store.quizzes?.steps?.step_3" class="col-xl-6 col-lg-6 col-12 mb-4">
                           <div class="input  d-flex flex-column gap-2">
                               <label for=""> {{ input?.name }} </label>
                               <Select  v-model="selectedStep3[`selected${index+1}`]" @update:modelValue="updateAnswer(input.id, $event)" v-if="input?.type == 'single'"  :options="input?.answers" style="min-height: 41px;" optionValue="id" optionLabel="name"
                                   placeholder="  " class="" />
                                   <InputText v-else name="" v-model="selectedStep3[`selected${index+1}`]"   @update:modelValue="updateAnswer(input.id, $event)"
                                   type="text" placeholder="" />
                   ...
                   <div v-if="selectedInputs == 4" class="">
                       <h4> {{ store.quizzes?.steps?.step_4[0]?.name }} </h4>
                       <div class="mt-4">
                           <div class="radio-check-inputs">
                               <label v-for="(theinput, index) in  store.quizzes?.steps?.step_4[0]?.answers" :key="`check-${index}`" :for="`check-${index}`"
                                   class="input d-flex align-items-center justify-content-between">
                                   <span>
                                     {{ theinput.name }}
                                   </span>
                                   <Checkbox v-model="selectedStep4"  :inputId="`check-${index}`" :value="theinput.id"    @change="updateAnswer(store.quizzes?.steps?.step_4[0]?.id, selectedStep4, true)"                                     />
                               </label>
                           </div>
                       </div>
                   </div>
                   <div v-if="selectedInputs == 5" class="">
                       <h4> {{ store.quizzes?.steps?.step_5[0]?.name }} </h4>
                       <div class="mt-4">
                           <div class="radio-check-inputs">
                               <label v-for="(theinput, index) in  store.quizzes?.steps?.step_5[0]?.answers" :key="`check-${index}`" :for="`check-${index}`"
                                   class="input d-flex align-items-center justify-content-between">
                                   <span>
                                     {{ theinput.name }}
                                   </span>
                                   <Checkbox v-model="selectedStep5"  :inputId="`check-${index}`" :value="theinput.id"    @change="updateAnswer(store.quizzes?.steps?.step_5[0]?.id, selectedStep5, true)"                                     />
                               </label>
                           </div>
                       </div>
                   </div>

                   <div v-if="selectedInputs == 6"  class="row">
                       <div v-for="input , index in store.quizzes?.steps?.step_6" class="col-xl-6 col-lg-6 col-12 mb-4">
                           <div class="input  d-flex flex-column gap-2">
                               <label for=""> {{ input?.name }} </label>
                               <Select  v-model="selectedStep6[`selected${index+1}`]" @update:modelValue="updateAnswer(input.id, $event)" v-if="input?.type == 'single'"  :options="input?.answers" style="min-height: 41px;" optionValue="id" optionLabel="name"
                                   placeholder="  " class="" />
                                   <InputText v-else name="" v-model="selectedStep6[`selected${index+1}`]"   @update:modelValue="updateAnswer(input.id, $event)"
                                   type="text" placeholder="" />
                   ...
                   <div v-if="selectedInputs == 7" class="">
                       <h4> {{ store.quizzes?.steps?.step_7[0]?.name }} </h4>
                       <div class="mt-4">
                           <div class="radio-check-inputs">
                               <label v-for="(theinput, index) in  store.quizzes?.steps?.step_7[0]?.answers" :key="`check-${index}`" :for="`check-${index}`"
                                   class="input d-flex align-items-center justify-content-between">
                                   <span>
                                     {{ theinput.name }}
                                   </span>
                                   <Checkbox v-model="selectedStep7"  :inputId="`check-${index}`" :value="theinput.id"    @change="updateAnswer(store.quizzes?.steps?.step_7[0]?.id, selectedStep7, true)"                                     />
                               </label>
                           </div>
                       </div>
                   </div>

                   <div v-if="selectedInputs == 8"  class="row">
                       <div v-for="input , index in store.quizzes?.steps?.step_8" class="col-xl-6 col-lg-6 col-12 mb-4">
                           <div class="input  d-flex flex-column gap-2">
                               <label for=""> {{ input?.name }} </label>
                               <Select  v-model="selectedStep8[`selected${index+1}`]" @update:modelValue="updateAnswer(input.id, $event)" v-if="input?.type == 'single'"  :options="input?.answers" style="min-height: 41px;" optionValue="id" optionLabel="name"
                                   placeholder="  " class="" />
                                   <InputText v-else name="" v-model="selectedStep8[`selected${index+1}`]"   @update:modelValue="updateAnswer(input.id, $event)"
                                   type="text" placeholder="" />
                   ...
                   <div class="d-flex align-items-center justify-content-between">
                       ...
                       <button v-if="selectedInputs != 9"  @click="nextStep()" 
                       :disabled="!isStepValid" class="bg-green text-light py-2 px-5 rounded-5 mt-5"> {{ selectedInputs <= 8 ? 'التالي' : 'انهاء' }}
                       </button>
                   </div>
               </div>
           </div>
...
const selectedStep5 = ref([]);
const selectedStep6 = ref({
   selected1: undefined, 
   selected2: undefined, 
   selected3: undefined, 
   selected4: undefined, 
})
const selectedStep7 = ref([]);
const selectedStep8 = ref({
   selected1: undefined, 
   selected2: undefined, 
   selected3: undefined, 
   selected4: undefined, 
...
const updatePercentage = computed(() => Math.round((questionNum.value / 25) * 100));
